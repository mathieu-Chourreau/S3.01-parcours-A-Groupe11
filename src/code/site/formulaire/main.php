...
$lIngredientsPref = $_SESSION['lIngredientsPref'];
...
$stmt = $conn->prepare("SELECT vege_pref, budget, tempsCuisine
                        FROM preferencesComplementaires 
...
if ($result->num_rows > 0) {
    ...
    $vege = $row['vege_pref'];
    $tempsCuisineMax = $row['tempsCuisine'];
    $budget = $row['budget'];
...
echo "<script>";
echo "console.log('Valeur de \$vege : $vege');";
echo "console.log('Valeur de \$tempsCuisineMax : $tempsCuisineMax');";
echo "console.log('Valeur de \$budget : $budget');";
echo "</script>";
